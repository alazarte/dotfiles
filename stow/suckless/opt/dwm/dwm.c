...
monocle(Monitor *m)
{
	unsigned int n = 0;
	Client *c;
	Gap *gap = selmon->pertag->gap[selmon->pertag->curtag];
...
tile(Monitor *m)
{
	...
	Client *c;
	Gap *gap = m->pertag->gap[m->pertag->curtag];
	...
	for (i = 0, my = ty = gap->gappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			h = (m->wh - my) / (MIN(n, m->nmaster) - i) - gap->gappx;
			resize(c, m->wx + gap->gappx, m->wy + my, mw - (2*c->bw) - gap->gappx, h - (2*c->bw), 0);
			if (my + HEIGHT(c) + gap->gappx < m->wh)
				my += HEIGHT(c) + gap->gappx;
		} else {
			h = (m->wh - ty) / (n - i) - gap->gappx;
			resize(c, m->wx + mw + gap->gappx, m->wy + ty, m->ww - mw - (2*c->bw) - 2*gap->gappx, h - (2*c->bw), 0);
			if (ty + HEIGHT(c) + gap->gappx < m->wh)
				ty += HEIGHT(c) + gap->gappx;
		}
}
